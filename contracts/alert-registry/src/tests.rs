...
fn test_register_alert_exactly_50_rules() {
    ...
    // (DuplicateRule, #15). The boundary that IS reachable is asserted in
    // `test_register_and_get_alert` / `test_duplicate_rules_rejected`.
    let mut rules: Vec<String> = vec![&env];
    for i in 0..50u32 {
        rules.push_back(str(
            &env,
...
fn test_alert_transfer_success() {
    ...
    let cfg = client.get_alert(&new_owner, &id).unwrap();
...
fn test_batch_register_alert_boundary_size() {
    ...
    let mut inputs: Vec<AlertInput> = vec![&env];
    // 12 is the largest batch that fits in one transaction's resource
    // footprint (each entry writes alert + two index entries).
    for _ in 0..12u32 {
        inputs.push_back(alert_input(&env, &owner, &target, "A"));
    }
...
fn test_batch_remove_alert_boundary_size() {
    ...
    let mut ids: Vec<u64> = vec![&env];
    // 10 removals fit one transaction's 100-entry footprint (each removal
    // touches the alert entry plus owner and contract indexes).
    for _ in 0..10u32 {
        let id =
            client.register_alert(&owner, &target, &str(&env, "A"), &hash64(&env), &vec![&env]);
        ids.push_back(id);
...
fn setup_with_watcher_registry() -> (
    ...
    // Both contracts are deployed through their constructors — the only init
    // path since soroban-sdk 25.
    let admin = Address::generate(&env);
...
fn test_bump_alert_clamps_to_max_ttl() {
    ...
    let bump_event = events.iter().find(|(_, topics, _)| {
        topics.len() == 2
            ...
            && Symbol::from_val(&env, &topics.get(1).unwrap()) == soroban_sdk::symbol_short!("bump")
    });
